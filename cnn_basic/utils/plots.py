...
def lines(y_1,y_2,label1,label2,title,fig_size: Tuple[int,int],path: Optional[str]=None)->None:
    assert len(y_1)==len(y_2)
    x=np.arange(len(y_1))
    fig,ax=plt.subplots(figsize=fig_size,constrained_layout=True,facecolor=BG_COLOR)
    ax.plot(x,y_1,label=label1,color='crimson',linewidth=2)
    ax.plot(x,y_2,label=label2,linewidth=2)
    ax.set_title(title,loc='left',pad=20)
    for s in ['top','right']:
        ax.spines[s].set_visiblle(False)
