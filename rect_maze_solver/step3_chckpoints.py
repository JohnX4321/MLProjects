...
def numberMaze(graph, initial, new,
               l):  ##This assigns numbers to the cells based on a certain algorithm that we have used
...
def OptimumPath(markers, initial, final,
                filePath):  ##This function checks the length of all the possible paths and assigns the shortest path length and returns that path as the final shortest optimum path
    shortest = range(5000)
...
def main(filePath, flag=0):
    ...
    pathArray = findOptimumPath(listOfMarkers.values(), initial_point, final_point,
                                filePath)  ## Acquire the list of paths for optimum traversal.
    print(pathArray)
    img = colourPath(imgBinary, pathArray,
                     listOfMarkers.values())  ## Highlight the whole optimum path in the maze image
